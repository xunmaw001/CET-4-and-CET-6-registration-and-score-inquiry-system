            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑考场</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 级联表的字段 -->
                                    <div class="form-group col-md-6">
                                        <label>考试</label>
                                        <div>
                                            <select id="kaoshiSelect" name="kaoshiSelect"
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>考试名</label>
                                        <input id="kaoshiName" name="kaoshiName" class="form-control"
                                               placeholder="考试名" readonly>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>考试类型</label>
                                        <input id="kaoshiValue" name="kaoshiValue" class="form-control"
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input id="kaoshiNumber" name="kaoshiNumber" class="form-control"
                                           onchange="kaoshiNumberChickValue(this)" value="0"  placeholder="考场人数" readonly>
                                </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
           ...
           if($("#laoshiId") !=null){
            ...
            httpJson("kaochang/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addkaochang');
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function kaoshiNumberChickValue(e){
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                kaoshiId: "required",
                laoshiId: "required",
                kaochangName: "required",
                kaoshiNumber: "required",
                kaochangAddress: "required",
            },
            messages: {
                kaoshiId: "考试不能为空",
                laoshiId: "监考老师不能为空",
                kaochangName: "考场名不能为空",
                kaoshiNumber: "考场人数不能为空",
                kaochangAddress: "考场地址不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addkaochang = window.sessionStorage.getItem("addkaochang");
        if (addkaochang != null && addkaochang != "" && addkaochang != "null") {
            window.sessionStorage.removeItem('addkaochang');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("kaochang/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    function kaoshiDataBind(){
        ...
        // startTimeDate.setTime();
        $("#startTime").val(ruleForm.startTime);//把时间戳转换为日期带时分秒格式
